...
func handleComments(w http.ResponseWriter, r *http.Request) {
	// Since multiple requests could come in at once,ensure we have a lock around all file operations
	commentMutex.Lock()
	...
	case "POST":
		...
		if err := cFile.Truncate(0); err != nil {
			http.Error(w, fmt.Sprintf("Unable to truncate data file (%s): %s", dataFile, err),
				http.StatusInternalServerError)
			return
		}

		if r, err := cFile.Seek(0, 0); r != 0 && err != nil {
			http.Error(w, fmt.Sprintf("Unable to seek to beginning of data file (%s): %s", dataFile, err),
				http.StatusInternalServerError)
			return
		}
